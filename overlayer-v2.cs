...
//
// Overlayer owns its layout and application behavior in this script. The shared
...
public class CPHInline
{
    ...
    public void Init()
    {
        // Dependency resolution is delayed until Execute() so the action still
        // compiles and can explain a missing shared UI DLL at runtime.
    }
...
public static class CrntlyDependencyBootstrap
{
        ...
        out CrntlyScriptWindowProxy window)
    {
        ...
        try
        {
            ...
            {
                var message =
                    "CRNTLY Overlayer needs CRNTLY.StreamerBot.UI.dll.\n\n" +
                    "Expected:\n" + dllPath + "\n\n" +
...
public sealed class CrntlyScriptWindowProxy : IDisposable
{
    ...
    public Action<string> EventRaised
    {
        set
        {
            var property = _bridgeType.GetProperty("EventRaised");
            if (property == null || !property.CanWrite)
                throw new MissingMemberException(_bridgeType.FullName, "EventRaised");
            property.SetValue(_bridge, value, null);
        }
    }

    public void Show(string xaml)
    {
        _show.Invoke(_bridge, new object[] { xaml });
    }

    public void Hide()
    {
        _hide.Invoke(_bridge, null);
    }
...
{
    private const string ResetIcon = "M12,5V2L8,6L12,10V7C15.31,7 18,9.69 18,13C18,16.31 15.31,19 12,19C8.69,19 6,16.31 6,13H4C4,17.42 7.58,21 12,21C16.42,21 20,17.42 20,13C20,8.58 16.42,5 12,5Z";

    private const string WindowXaml = @"
<Window xmlns=""http://schemas.microsoft.com/winfx/2006/xaml/presentation""
        xmlns:x=""http://schemas.microsoft.com/winfx/2006/xaml""
        xmlns:shell=""clr-namespace:System.Windows.Shell;assembly=PresentationFramework""
        Title=""CRNTLY • Overlayer""
        Width=""780"" Height=""440"" MinWidth=""700"" MinHeight=""390""
  ...
  <shell:WindowChrome.WindowChrome>
    <shell:WindowChrome CaptionHeight=""0"" ResizeBorderThickness=""4""
                        CornerRadius=""3"" GlassFrameThickness=""0"" />
  </shell:WindowChrome.WindowChrome>

  <Border Background=""{DynamicResource Crntly.Background}""
          BorderBrush=""{DynamicResource Crntly.Border}""
          BorderThickness=""1""
          CornerRadius=""{DynamicResource Crntly.WindowRadius}"">
    <Grid>
      <Grid.RowDefinitions>
        <RowDefinition Height=""{DynamicResource Crntly.TitleBarHeight}"" />
      ...
      <Border x:Name=""TitleBar"" Grid.Row=""0""
              Background=""{DynamicResource Crntly.Surface}""
              BorderBrush=""{DynamicResource Crntly.Border}""
              BorderThickness=""0,0,0,1"" CornerRadius=""3,3,0,0"">
        <Grid Margin=""7,0,3,0"">
          ...
          <StackPanel Orientation=""Horizontal"" VerticalAlignment=""Center"">
            <TextBlock Text=""CRNTLY"" Foreground=""{DynamicResource Crntly.TextMuted}""
                       FontWeight=""SemiBold"" FontSize=""9"" />
            <TextBlock Text=""  /  Overlayer"" Foreground=""{DynamicResource Crntly.Text}""
                       FontWeight=""SemiBold"" FontSize=""10"" />
          </StackPanel>
          <StackPanel Grid.Column=""1"" Orientation=""Horizontal"" Margin=""0,0,6,0""
                      VerticalAlignment=""Center"">
            <Ellipse x:Name=""ServerDot"" Width=""5"" Height=""5""
                     Fill=""{DynamicResource Crntly.TextMuted}"" Margin=""0,0,5,0"" />
            <TextBlock x:Name=""ServerBadgeText"" Text=""SERVER OFFLINE""
                       Foreground=""{DynamicResource Crntly.TextMuted}""
                       FontSize=""8"" FontWeight=""SemiBold"" />
          </StackPanel>
          <Button x:Name=""MinimizeButton"" Grid.Column=""2"" Content=""—""
                  Style=""{StaticResource Crntly.TitleButton}""
                  shell:WindowChrome.IsHitTestVisibleInChrome=""True"" />
          <Button x:Name=""CloseButton"" Grid.Column=""3"" Content=""×""
                  Style=""{StaticResource Crntly.TitleButton}""
                  shell:WindowChrome.IsHitTestVisibleInChrome=""True"" />
        </Grid>
      </Border>

      <Border Grid.Row=""1"" Style=""{StaticResource Crntly.Card}""
              Margin=""7,5,7,4"" Padding=""5"">
        <Grid>
          <Grid.ColumnDefinitions>
            <ColumnDefinition Width=""Auto"" />
          ...
          <Button x:Name=""ServerButton"" Grid.Column=""0"" Content=""Start server""
                  Style=""{StaticResource Crntly.PrimaryButton}"" MinWidth=""84"" />
          <TextBox x:Name=""ServerUrlBox"" Grid.Column=""2"" IsReadOnly=""True""
                   Text=""http://localhost:42069/"" VerticalContentAlignment=""Center"" />
          <Button x:Name=""CopyUrlButton"" Grid.Column=""4""
                  Style=""{StaticResource Crntly.IconButton}"" ToolTip=""Copy compositor URL"">
            <Path Style=""{StaticResource Crntly.IconPath}""
                  Data=""M19,21H8C6.9,21 6,20.1 6,19V8C6,6.9 6.9,6 8,6H19C20.1,6 21,6.9 21,8V19C21,20.1 20.1,21 19,21 M16,3H5C3.9,3 3,3.9 3,5V16H5V5H16V3Z"" />
          </Button>
          <Button x:Name=""OpenPreviewButton"" Grid.Column=""6""
                  Style=""{StaticResource Crntly.IconButton}"" ToolTip=""Open compositor preview in browser"">
            <Path Style=""{StaticResource Crntly.IconPath}""
                  Data=""M14,3V5H17.59L7.76,14.83L9.17,16.24L19,6.41V10H21V3H14 M19,19H5V5H12V3H5C3.9,3 3,3.9 3,5V19C3,20.1 3.9,21 5,21H19C20.1,21 21,20.1 21,19V12H19V19Z"" />
          </Button>
        </Grid>
      </Border>
        ...
        <Border Grid.Column=""0"" Style=""{StaticResource Crntly.Card}"" Padding=""6"">
          <Grid>
            ...
            <Grid Grid.Row=""0"">
              ...
              <TextBlock Text=""Overlays"" Foreground=""{DynamicResource Crntly.Text}""
                         FontWeight=""SemiBold"" FontSize=""11"" VerticalAlignment=""Center"" />
              <StackPanel Grid.Column=""1"" Orientation=""Horizontal"" VerticalAlignment=""Center"">
                <Button x:Name=""AddButton"" Style=""{StaticResource Crntly.IconButton}"" ToolTip=""Add overlay"">
                  <Path Style=""{StaticResource Crntly.IconPath}"" Data=""M11,5H13V11H19V13H13V19H11V13H5V11H11V5Z"" />
                </Button>
                <Button x:Name=""DuplicateButton"" Style=""{StaticResource Crntly.IconButton}"" ToolTip=""Duplicate selected overlay"">
                ...
                <Button x:Name=""MoveUpButton"" Style=""{StaticResource Crntly.IconButton}"" ToolTip=""Move overlay up"">
                  <Path Style=""{StaticResource Crntly.IconPath}"" Data=""M7,14L12,9L17,14H7Z"" />
                </Button>
                <Button x:Name=""MoveDownButton"" Style=""{StaticResource Crntly.IconButton}"" ToolTip=""Move overlay down"">
                  <Path Style=""{StaticResource Crntly.IconPath}"" Data=""M7,10L12,15L17,10H7Z"" />
                </Button>
                <Button x:Name=""DeleteButton"" Style=""{StaticResource Crntly.DangerIconButton}"" ToolTip=""Delete selected overlay"">
                  <Path Style=""{StaticResource Crntly.IconPath}"" Data=""M19,4H15.5L14.5,3H9.5L8.5,4H5V6H19V4 M6,19C6,20.1 6.9,21 8,21H16C17.1,21 18,20.1 18,19V7H6V19Z"" />
                </Button>
              </StackPanel>
            </Grid>
                ...
                <DataTemplate>
                  <Grid>
                    ...
                    <StackPanel Grid.Column=""1"" VerticalAlignment=""Center"">
                      <TextBlock Text=""{Binding Name}"" Foreground=""{DynamicResource Crntly.Text}""
                                 FontWeight=""SemiBold"" FontSize=""10"" TextTrimming=""CharacterEllipsis"" />
                      <TextBlock Text=""{Binding Url}"" Foreground=""{DynamicResource Crntly.TextSubtle}""
                                 FontSize=""8"" Margin=""0,1,5,0"" TextTrimming=""CharacterEllipsis"" />
                    </StackPanel>
                    <TextBlock Grid.Column=""2"" Text=""{Binding DisplaySourceKind}""
                               Foreground=""{DynamicResource Crntly.TextSubtle}""
                               FontSize=""7"" FontWeight=""SemiBold"" VerticalAlignment=""Top"" Margin=""3,1,0,0"" />
                  </Grid>
                </DataTemplate>
              </ListBox.ItemTemplate>
            </ListBox>
          ...
          <ScrollViewer VerticalScrollBarVisibility=""Auto"" HorizontalScrollBarVisibility=""Disabled"">
            <StackPanel Margin=""0,0,2,0"">
              <Grid Margin=""0,0,0,6"">
                ...
                <StackPanel Orientation=""Horizontal"" VerticalAlignment=""Center"">
                  <TextBlock Text=""Overlay settings"" Foreground=""{DynamicResource Crntly.Text}""
                             FontWeight=""SemiBold"" FontSize=""12"" />
                  <StackPanel Orientation=""Horizontal"" Margin=""7,0,0,0"" VerticalAlignment=""Center"">
                    <Ellipse x:Name=""EditorStatusDot"" Width=""4"" Height=""4""
                             Fill=""{DynamicResource Crntly.TextMuted}"" Margin=""0,0,4,0"" />
                    <TextBlock x:Name=""EditorStatusText"" Text=""Autosave""
                               Foreground=""{DynamicResource Crntly.TextMuted}"" FontSize=""8"" />
                  </StackPanel>
                </StackPanel>
                <StackPanel Grid.Column=""1"" Orientation=""Horizontal"" VerticalAlignment=""Center"">
                  <Button x:Name=""ResetLayoutButton"" Style=""{StaticResource Crntly.IconButton}""
                          Margin=""0,0,5,0"" ToolTip=""Reset size and position to defaults"">
                    <Path Style=""{StaticResource Crntly.IconPath}"" Data=""M12,5V2L8,6L12,10V7C15.31,7 18,9.69 18,13C18,16.31 15.31,19 12,19C8.69,19 6,16.31 6,13H4C4,17.42 7.58,21 12,21C16.42,21 20,17.42 20,13C20,8.58 16.42,5 12,5Z"" />
                  </Button>
              ...
              <Grid Margin=""0,0,0,6"">
                ...
                <StackPanel Grid.Column=""0"">
                  <TextBlock Text=""NAME"" Style=""{StaticResource Crntly.Label}"" Margin=""0,0,0,2"" />
                  <TextBox x:Name=""NameBox"" />
                </StackPanel>
                <StackPanel Grid.Column=""2"">
                  <TextBlock Text=""URL / LOCAL FILE"" Style=""{StaticResource Crntly.Label}"" Margin=""0,0,0,2"" />
                  <Grid>
                    <Grid.ColumnDefinitions>
                      <ColumnDefinition Width=""*"" />
                    ...
                    <Button x:Name=""OpenSourceButton"" Grid.Column=""2""
                            Style=""{StaticResource Crntly.IconButton}"" ToolTip=""Open overlay source in browser"">
                      <Path Style=""{StaticResource Crntly.IconPath}""
                            Data=""M14,3V5H17.59L7.76,14.83L9.17,16.24L19,6.41V10H21V3H14 M19,19H5V5H12V3H5C3.9,3 3,3.9 3,5V19C3,20.1 3.9,21 5,21H19C20.1,21 21,20.1 21,19V12H19V19Z"" />
                    </Button>
                  </Grid>
              ...
              <Grid Margin=""0,0,0,6"">
                ...
                <StackPanel Grid.Column=""0"">
                  <TextBlock Text=""WIDTH"" Style=""{StaticResource Crntly.Label}"" Margin=""0,0,0,2"" />
                  <Grid>
                    <Grid.ColumnDefinitions><ColumnDefinition Width=""*"" /><ColumnDefinition Width=""3"" /><ColumnDefinition Width=""22"" /></Grid.ColumnDefinitions>
                    <TextBox x:Name=""WidthBox"" />
                    <Button x:Name=""ResetWidthButton"" Grid.Column=""2"" Style=""{StaticResource Crntly.IconButton}"" ToolTip=""Reset width to 100%"">
                      <Path Style=""{StaticResource Crntly.IconPath}"" Data=""M12,5V2L8,6L12,10V7C15.31,7 18,9.69 18,13C18,16.31 15.31,19 12,19C8.69,19 6,16.31 6,13H4C4,17.42 7.58,21 12,21C16.42,21 20,17.42 20,13C20,8.58 16.42,5 12,5Z"" />
                    </Button>
                  </Grid>
                </StackPanel>
                <StackPanel Grid.Column=""2"">
                  <TextBlock Text=""HEIGHT"" Style=""{StaticResource Crntly.Label}"" Margin=""0,0,0,2"" />
                  <Grid>
                    <Grid.ColumnDefinitions><ColumnDefinition Width=""*"" /><ColumnDefinition Width=""3"" /><ColumnDefinition Width=""22"" /></Grid.ColumnDefinitions>
                    <TextBox x:Name=""HeightBox"" />
                    <Button x:Name=""ResetHeightButton"" Grid.Column=""2"" Style=""{StaticResource Crntly.IconButton}"" ToolTip=""Reset height to 100%"">
                      <Path Style=""{StaticResource Crntly.IconPath}"" Data=""M12,5V2L8,6L12,10V7C15.31,7 18,9.69 18,13C18,16.31 15.31,19 12,19C8.69,19 6,16.31 6,13H4C4,17.42 7.58,21 12,21C16.42,21 20,17.42 20,13C20,8.58 16.42,5 12,5Z"" />
                    </Button>
                  </Grid>
                </StackPanel>
              ...
              <Border Style=""{StaticResource Crntly.SubtleCard}"" Padding=""6"">
                <StackPanel>
                  <StackPanel Orientation=""Horizontal"" Margin=""0,0,0,4"">
                    <TextBlock Text=""Position"" Foreground=""{DynamicResource Crntly.Text}"" FontWeight=""SemiBold"" FontSize=""10"" />
                    <TextBlock Text=""  ·  live"" Foreground=""{DynamicResource Crntly.TextSubtle}"" FontSize=""8"" />
                  </StackPanel>
                  <Grid Margin=""0,0,0,4"">
                    <Grid.ColumnDefinitions>
                      <ColumnDefinition Width=""14"" /><ColumnDefinition Width=""*"" /><ColumnDefinition Width=""6"" />
                      <ColumnDefinition Width=""58"" /><ColumnDefinition Width=""3"" /><ColumnDefinition Width=""22"" />
                    </Grid.ColumnDefinitions>
                    ...
                    <Slider Grid.Column=""1"" x:Name=""LeftSlider"" Minimum=""-1920"" Maximum=""3840""
                            SmallChange=""1"" LargeChange=""50"" IsMoveToPointEnabled=""True""
                            ToolTip=""X position in pixels. Range: -1920 to 3840."" />
                    <TextBox Grid.Column=""3"" x:Name=""LeftBox"" TextAlignment=""Right"" />
                    <Button x:Name=""ResetLeftButton"" Grid.Column=""5"" Style=""{StaticResource Crntly.IconButton}"" ToolTip=""Reset X to 0"">
                      <Path Style=""{StaticResource Crntly.IconPath}"" Data=""M12,5V2L8,6L12,10V7C15.31,7 18,9.69 18,13C18,16.31 15.31,19 12,19C8.69,19 6,16.31 6,13H4C4,17.42 7.58,21 12,21C16.42,21 20,17.42 20,13C20,8.58 16.42,5 12,5Z"" />
                    </Button>
                  </Grid>
                  <Grid>
                    ...
                    <Slider Grid.Column=""1"" x:Name=""TopSlider"" Minimum=""-1080"" Maximum=""2160""
                            SmallChange=""1"" LargeChange=""50"" IsMoveToPointEnabled=""True""
                            ToolTip=""Y position in pixels. Range: -1080 to 2160."" />
                    <TextBox Grid.Column=""3"" x:Name=""TopBox"" TextAlignment=""Right"" />
                    <Button x:Name=""ResetTopButton"" Grid.Column=""5"" Style=""{StaticResource Crntly.IconButton}"" ToolTip=""Reset Y to 0"">
                      <Path Style=""{StaticResource Crntly.IconPath}"" Data=""M12,5V2L8,6L12,10V7C15.31,7 18,9.69 18,13C18,16.31 15.31,19 12,19C8.69,19 6,16.31 6,13H4C4,17.42 7.58,21 12,21C16.42,21 20,17.42 20,13C20,8.58 16.42,5 12,5Z"" />
                    </Button>
                  </Grid>
                </StackPanel>
      ...
      <Grid Grid.Row=""3"" Margin=""8,0"">
        <TextBlock Text=""CRNTLY • livestreaming.tools"" Foreground=""{DynamicResource Crntly.TextSubtle}"" FontSize=""7"" VerticalAlignment=""Center"" />
        <TextBlock Text=""single-source compositor"" Foreground=""{DynamicResource Crntly.TextSubtle}"" FontSize=""7"" HorizontalAlignment=""Right"" VerticalAlignment=""Center"" />
      </Grid>
    </Grid>
  </Border>
</Window>";
    ...
    private void OnUiEvent(string key)
    {
        ...
        try
        {
            ...
            {
                case "drag":
                    try { _window.InvokeMethod("$window", "DragMove"); } catch { }
                    break;
                case "minimize":
                ...
                case "server":
                    FlushPendingAutosave();
                    if (string.Equals(_window.Get<string>("ServerButton", "Content", ""), "Stop server", StringComparison.OrdinalIgnoreCase))
                        SafeInvoke(StopServerRequested);
                    else
                        SafeInvoke(StartServerRequested);
                    break;
                case "copy-url":
                    TrySetClipboard(_window.Get<string>("ServerUrlBox", "Text", ""));
                    break;
                case "open-preview":
                    OpenExternal(_window.Get<string>("ServerUrlBox", "Text", ""), false);
                    break;
                case "open-source":
                    if (!OpenExternal(_window.Get<string>("UrlBox", "Text", ""), true))
    ...
    private void AddOverlay()
    {
        ...
        _items.Add(item);
        _window.RefreshItems("OverlayList");
    ...
    private void SliderChanged(string sliderName, string textBoxName)
    {
        ...
        try
        {
            var value = _window.Get<double>(sliderName, "Value", 0);
            _window.SetProperty(textBoxName, "Text", FormatPositionNumber(value));
        }
        finally
        {
    ...
    private void ResetField(string textBoxName, string value, string sliderName)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(sliderName))
                SyncPositionSlider(textBoxName, sliderName);
        }
        finally
        {
    ...
    private void PreviewPosition()
    {
        ...
        string left;
        string top;
        if (!TryNormalizePosition(Text("LeftBox"), "0px", out left) ||
            !TryNormalizePosition(Text("TopBox"), "0px", out top))
            return;
    ...
    private bool TryValidateEditor(out string width, out string height, out string left, out string top, out string message)
    {
        ...
        Uri uri;
        if (!TryGetSupportedUri(Text("UrlBox"), out uri))
        {
            message = "Enter a valid URL";
            return false;
        }

        if (!TryNormalizeCssLength(Text("WidthBox"), "100%", false, out width) ||
            !TryNormalizeCssLength(Text("HeightBox"), "100%", false, out height))
        {
    ...
    {
        _window.SetProperty("NameBox", "Text", "");
        _window.SetProperty("UrlBox", "Text", "");
        _window.SetProperty("WidthBox", "Text", "100%");
        _window.SetProperty("HeightBox", "Text", "100%");
        _window.SetProperty("LeftBox", "Text", "0");
        _window.SetProperty("TopBox", "Text", "0");
        _window.SetProperty("EnabledBox", "IsChecked", true);
        _window.SetProperty("LeftSlider", "Value", 0d);
        _window.SetProperty("TopSlider", "Value", 0d);
        SetEditorStatus("Autosave", "Crntly.TextMuted");
    }
    ...
    private void SetEditorEnabled(bool enabled)
    {
        ...
        {
            "NameBox", "UrlBox", "WidthBox", "HeightBox", "LeftBox", "TopBox",
            "EnabledBox", "ResetLayoutButton", "ResetWidthButton", "ResetHeightButton",
            "ResetLeftButton", "ResetTopButton"
        })
            _window.SetProperty(name, "IsEnabled", enabled);

        _window.SetProperty("OpenSourceButton", "IsEnabled", enabled && CanOpen(Text("UrlBox")));
        _window.SetProperty("LeftSlider", "IsEnabled", enabled);
        _window.SetProperty("TopSlider", "IsEnabled", enabled);
    }

    private void SyncPositionSlider(string textBox, string slider)
    {
        double pixels;
        if (!TryParsePixelPosition(Text(textBox), out pixels))
        {
            _window.SetProperty(slider, "IsEnabled", false);
            return;
        }

        _window.SetProperty(slider, "IsEnabled", _editingItem != null);
        var min = _window.Get<double>(slider, "Minimum", 0);
        var max = _window.Get<double>(slider, "Maximum", 0);
        _syncingPosition = true;
        try
        {
            _window.SetProperty(slider, "Value", Math.Max(min, Math.Min(max, pixels)));
        }
        finally
        {
            _syncingPosition = false;
    ...
    private OverlayRecord SelectedItem()
    {
        return _window.GetSelectedItem("OverlayList") as OverlayRecord;
    }

    private string Text(string control)
    {
        return _window.Get<string>(control, "Text", "") ?? "";
    }
    ...
    private void RaiseOrderChanged()
    {
        ...
        if (callback != null)
            callback(_items.Select(x => x.Clone()).ToList());
    }
    ...
    private static void SafeInvoke(Action<OverlayRecord> callback, OverlayRecord item)
    {
        if (callback != null)
            callback(item);
    }
    ...
    {
        Uri uri;
        return TryGetSupportedUri(value, out uri);
    }
    ...
    private static bool Confirm(string title, string message)
    {
        try
        {
            ...
            if (messageBox == null || buttonsType == null || iconType == null)
                return true;
            ...
            var yesNo = Enum.Parse(buttonsType, "YesNo");
            var question = Enum.Parse(iconType, "Question");
            var result = method.Invoke(null, new[] { message, title, yesNo, question });
    ...
    private static bool TryNormalizePosition(string value, string fallback, out string normalized)
    {
        ...
        var input = value.Trim().ToLowerInvariant();
        double bare;
        if (TryParseNumber(input, out bare))
        {
            normalized = FormatNumber(bare) + "px";
            return true;
        }
    ...
    private static string DisplayPosition(string value)
    {
        ...
        return input.EndsWith("px", StringComparison.OrdinalIgnoreCase)
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException("OverlayerScriptUi");
    }
...
public sealed class OverlayRecord
{
    ...
    public string DisplaySourceKind
    {
        get
        {
            Uri uri;
            return Uri.TryCreate(Url, UriKind.Absolute, out uri) && uri.IsFile ? "LOCAL" : "WEB";
        }
    }
...
public sealed class OverlayerConfigStore
{
    ...
    private static void Append(List<OrderedOverlay> output, List<Dictionary<string, string>> rows, bool enabled, int fallbackOffset)
    {
        ...
        for (var i = 0; i < rows.Count; i++)
        {
            ...
            int order;
            if (!int.TryParse(Get(row, "Order", null), out order)) order = int.MaxValue;
...
<title>CRNTLY Overlayer</title><style>html,body,#crntly-root{margin:0;width:100%;height:100%;overflow:hidden;background:transparent;}#crntly-root{position:relative;}.crntly-overlay{position:absolute;border:0;margin:0;padding:0;overflow:hidden;background:transparent;}</style></head>
<body><div id=""crntly-root""></div><script>
(() => {
 const root=document.getElementById('crntly-root'); let lastState=''; let disconnectTimer=null; let events=null;
 function applyState(text){ if(text===lastState)return; lastState=text; let items; try{items=JSON.parse(text);}catch(_){return;} const keep=new Set();
  for(const item of items){const domId='ov-'+item.id; keep.add(domId); let frame=document.getElementById(domId); if(!frame){frame=document.createElement('iframe');frame.id=domId;frame.className='crntly-overlay';frame.scrolling='no';frame.allow='autoplay';root.appendChild(frame);} if(frame.dataset.src!==item.src){frame.dataset.src=item.src;frame.src=item.src;} frame.style.width=item.width;frame.style.height=item.height;frame.style.top=item.top;frame.style.left=item.left;root.appendChild(frame);} for(const frame of Array.from(root.children)){if(!keep.has(frame.id))frame.remove();}}
 function clearOutput(){applyState('[]');}
 async function sync(){try{const response=await fetch('/state',{cache:'no-store'});if(response.ok)applyState(await response.text());}catch(_){}}
 sync();
 if(window.EventSource){try{events=new EventSource('/events'); events.onopen=()=>{if(disconnectTimer){clearTimeout(disconnectTimer);disconnectTimer=null;}}; events.onmessage=event=>applyState(event.data); events.addEventListener('shutdown',()=>{if(disconnectTimer){clearTimeout(disconnectTimer);disconnectTimer=null;}clearOutput();}); events.onerror=()=>{if(disconnectTimer)clearTimeout(disconnectTimer);disconnectTimer=setTimeout(()=>{if(!events||events.readyState!==EventSource.OPEN)clearOutput();},350);};}catch(_){}}
 setInterval(sync,5000);
    ...
    public void UpdateItems(IList<OverlayRecord> items)
    {
        ...
        foreach (var item in items ?? new List<OverlayRecord>())
        {
            ...
            Uri uri;
            if (!Uri.TryCreate(source, UriKind.Absolute, out uri)) continue;
            if (uri.IsFile)
            {
            ...
            {
                { "id", item.Id }, { "src", source },
                { "width", SafeCss(item.Width, "100%") }, { "height", SafeCss(item.Height, "100%") },
                { "top", SafeCss(item.Top, "0px") }, { "left", SafeCss(item.Left, "0px") }
            });
        }
        var json = JsonConvert.SerializeObject(state, Formatting.None);
        lock (_gate) { _stateJson = json; _localRoots = localRoots; }
        BroadcastState(json);
    }
    ...
    public void Start()
    {
        ...
        try
        {
            main = new HttpListener(); main.Prefixes.Add(RootUrl); main.Start();
            local = new HttpListener(); local.Prefixes.Add(LocalUrl); local.Start();
            _listener = main; _localListener = local; IsRunning = true;
        ...
        catch
        {
            ...
            _listener = null; _localListener = null; IsRunning = false;
            throw;
        }
    }
    ...
    public void Stop()
    {
        ...
        if (!IsRunning && main == null && local == null) return;

        List<HttpListenerResponse> eventClients;
        lock (_gate)
        {
            _stateJson = "[]";
            _localRoots = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
            eventClients = _eventClients.ToList();
        }
        foreach (var response in eventClients)
        {
            try { WriteEvent(response, "[]", "shutdown"); }
            catch { RemoveEventClient(response); }
        }
    ...
    {
        var listener = ar.AsyncState as HttpListener; if (listener == null) return;
        HttpListenerContext context;
        try { context = listener.EndGetContext(ar); } catch { return; }
    ...
    {
        try { if (listener.IsListening) listener.BeginGetContext(callback, listener); } catch { }
    }
    ...
    private void HandleMain(HttpListenerContext context)
    {
        ...
        {
            string json; lock (_gate) json = _stateJson;
            WriteText(context, json, "application/json; charset=utf-8", HttpStatusCode.OK); return;
        }
    ...
    private void HandleEventStream(HttpListenerContext context)
    {
        ...
        response.SendChunked = true; response.KeepAlive = true; response.Headers["Cache-Control"] = "no-cache";
        string json;
        lock (_gate) { json = _stateJson; _eventClients.Add(response); }
        try { WriteRetry(response, 750); WriteEvent(response, json, null); }
        catch { RemoveEventClient(response); }
    }
    ...
    {
        if (!IsRunning) return;
        List<HttpListenerResponse> clients; lock (_gate) clients = _eventClients.ToList();
        foreach (var response in clients)
        {
            try { WriteEvent(response, json, null); } catch { RemoveEventClient(response); }
        }
    }
    ...
    private static void WriteEvent(HttpListenerResponse response, string json, string eventName)
    {
        ...
        if (!string.IsNullOrWhiteSpace(eventName)) builder.Append("event: ").Append(eventName).Append('\n');
    ...
    {
        lock (_gate) _eventClients.Remove(response);
        SafeClose(response);
    }
    ...
    private void HandleLocal(HttpListenerContext context)
    {
        ...
        {
            WriteText(context, "Not found", "text/plain; charset=utf-8", HttpStatusCode.NotFound); return;
        }
        ...
        try
        {
            using (var stream = new FileStream(candidate, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                ...
                if (!string.Equals(context.Request.HttpMethod, "HEAD", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    while ((read = stream.Read(buffer, 0, buffer.Length)) > 0) response.OutputStream.Write(buffer, 0, read);
                }
            }
        }
    ...
    private static void WriteText(HttpListenerContext context, string text, string contentType, HttpStatusCode status)
    {
        ...
        try
        {
            response.StatusCode = (int)status; response.ContentType = contentType; response.ContentLength64 = buffer.Length;
            response.Headers["Cache-Control"] = "no-store";
            if (!string.Equals(context.Request.HttpMethod, "HEAD", StringComparison.OrdinalIgnoreCase)) response.OutputStream.Write(buffer, 0, buffer.Length);
        }
        finally { SafeClose(response); }
    }
